...
class RegisterFrame(object):
    """
    An object representing a single register frame.

    NOTE: The RegisterFrame object must be passed a
    RegisterFrameDescription object.
    """
    __slots__ = ('magic',
                 'framereceivedms',
                 'rfd',
    ...
    def __init__(self, header):
        ...
        if self.magic is not 'F':
            raise HKEMagicError
        self.framecount = self.reader.uint32()
        self.framereceivedms = self.reader.uint32()
        self.registerlist = [Register(rd) for rd in
                             self.rfd._rdlist]
        self.registers = dict(zip(self.rfd._rkeylist, self.registerlist))
        self.endpos = self.reader.bitstream.pos + 0
        self.length = self.endpos - self.startpos + 1
...
class Register(object):
    ...
    def __init__(self, registerdescription):
        # This check slows down the read considerably, so just enforce
        # that hkebreader is actually a HKEBinaryReader object.
        # if isinstance(hkebreader, HKEBinaryReader):
        #     h = hkebreader
        # else:
        #     h = HKEBinaryReader(bitstream=hkebreader)
        self.rd = registerdescription
        self.registerdescription = self.rd
        self.reader = self.rd.reader

        self.startpos = self.reader.bitstream.pos + 0
        self.registerdescription = registerdescription
        self.rd = registerdescription

        self.name = self.rd.name
        self.fullname = self.rd.fullname
        self.columnname = self.rd.columnname
        self.nch = self.rd.nch
        self.nsamples = self.rd.nsamples
        self.rtype = self.rd.registertypename
        self.flags = self.rd.flags
        self.hasreduced = True if (self.flags is 4) else False
        self.haslinreduced = True if (self.flags is 2) else False
        self.linslope = self.rd.linslope
        self.linoffset = self.rd.linoffset
        self.rawdata = self.reader.array(self.nch*self.nsamples,
                                         self.rtype)
        if self.hasreduced:
            self.reduceddata = self.reader.floatarray(self.nch*self.nsamples)
        elif self.haslinreduced:
            self.reduceddata = [self.linslope*value + self.linoffset
...
class HKEBinaryReader(object):
    ...
    def __init__(self, bitstream=None, filename=None):
        ...
        if (filename is None) and (bitstream is None):
            return
        if (filename is not None) and (bitstream is not None):
            print "Two binary streams specified. Doesn't know which to\
 use, so no binary stream set."
            return
...
class Data(object):
    ...
    """
    def __init__(self, header):
        self.registerframelist = []
        self.registerframes = {}
        self.header = header
        self.reader = self.header.reader
        self.startpos = self.reader.bitstream.pos + 0
        if self.startpos != (self.header.length - 1):
            print "Data section beginning at unexpected place in file..."
            print "Header length = {hl}".format(hl=self.header.length)
            print "Data section start = {ds}".format(ds=self.startpos)
        try:
            while True:
                rf = RegisterFrame(self.header)
                framecount = rf.framecount

                self.registerframelist.append(rf)
                self.registerframes[framecount] = rf
        except ReadError:
            print "Read to end of data file."
            self.endpos = self.reader.bitstream.pos + 0
            self.length = self.endpos - self.startpos + 1
            return
